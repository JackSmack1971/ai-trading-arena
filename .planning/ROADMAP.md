...
- [x] **Phase 1: Foundation** - Fix two risk-gate correctness bugs, implement packages/telemetry, and harden event payload parsing *(Planned)*
...
Plans:
- [ ] 01-01-PLAN.md � Core schema additions: STRATEGY_SWITCH_REQUESTED event type, SimEventPayloadSchemas dispatch map, MarketFeedAdapter.onError interface (Wave 1)
- [ ] 01-02-PLAN.md � DB windowed query helpers: countOrdersInWindow, countStrategySwitchesInWindow (Wave 2)
- [ ] 01-03-PLAN.md � API + worker call site fixes: JSON.parse cast removal, real risk counter wiring, unsafe adapter cast removal (Wave 3)
- [ ] 01-04-PLAN.md � packages/telemetry: Pino logger factory with redaction, OTel naming constants (Wave 2, parallel with 01-02)
- [ ] 01-05-PLAN.md � db:validate smoke-test script, phase gate verification (Wave 4)
